...
class Board {
    ...
    drawBoard(canvas) {
      ...
      for (let i = 0; i < this.rows.length; i++) {
        for (let j = 0; j < this.columns.length; j++) {
          ...
          if(position === this.gameState.lastMove?.sourcePosition || position === this.gameState.lastMove?.targetPosition){
      ...
      if(this.gameState.pieces){
          ...
          for (let i = 0; i < keysBoard.length; i++) {
            ...
            if(chessPiece.complete){
              if(this.gameState.movablePieces.includes(keysBoard[i]) && this.gameState.turn === "white"){
    ...
    clickCanvas(){
      this.canvas.onclick = (e) => {
        ...
        if(!this.gameState.loading && "white" === this.gameState.turn){
          if(this.selectedPiece && this.gameState.moves[this.selectedPiece].includes(position)){
            this.gameState.takeTurn(this.selectedPiece, position);
            this.selectedPiece = undefined;
          }
        
        else if(Object.keys(this.gameState.pieces).includes(position) && this.checkPieceOwner(this.gameState.pieces[position]) === "white"){
    ...
    drawPossiblePath(){
      ...
      if(moves){
        ...
        for(let i = 0; i < moves.length; i++){
              ...
              if(this.gameState.pieces[moves[i]] && this.checkPieceOwner(this.gameState.pieces[moves[i]]) !== this.gameState.turn){
